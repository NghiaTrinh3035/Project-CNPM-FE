...
export const OwnerProductDetailPage = () => {
  ...
  const query = useQuery({
	queryKey: ["owner-product-detail", id],
  ...
  return (
	<Card>
	  <CardHeader className="space-y-3">
		...
		<CardTitle>Chi tiết sản phẩm (Owner)</CardTitle>
	  </CardHeader>
	  <CardContent>
		{!query.isLoading && !product ? (
		  <p className="text-sm text-muted-foreground">Không tìm thấy sản phẩm. <button className="underline" onClick={() => navigate(ROUTES.owner.products)}>Về danh sách</button></p>
		) : null}
